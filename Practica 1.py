...
# Función para obtener la lista de puertos seriales
...
# Función para establecer conexión con puerto
def conectar_serial():
...
notebook.add(pestana_conexion, text="Conexión")
notebook.add(pestana_control, text="Control")
notebook.add(pestana_opciones, text="Opciones")
notebook.pack(expand=1, fill="both")

# Imágenes para los estados del LED
img_led_verde = ImageTk.PhotoImage(Image.open("ledverde.png").resize((50, 50)))
img_led_rojo = ImageTk.PhotoImage(Image.open("ledrojo.png").resize((50, 50)))

# --- Pestaña de conexión ---
lbl_puerto = ttk.Label(pestana_conexion, text="Puerto Serial:")
...
btn_conectar.grid(row=1, column=0, columnspan=4, padx=10, pady=10)
...
btn_led_on.grid(row=2, column=0, padx=10, pady=10)

btn_led_off = ttk.Button(pestana_conexion, text="Apagar LED", command=desactivar_led)
btn_led_off.grid(row=2, column=1, padx=10, pady=10)
